...
def all_feats(scores, ordered=None):
    """
    ...
    if ordered == 'rank':
        ...
        scores = scores.sort_values('abs_diff', ascending=False)
...
def get_feedback(scores, ordered=None):
    """
    ...
    if ordered == 'random':
        for i in ordered_features:
            diff = scores['diff'].loc[scores['Features'] == i].to_list()[0]
            choice = random.choice(['imp', 'recog'])
            feedback.append(responses.fancy_responses[i][choice])
...
def extract_features(text, ordered=None):
    ...
    if ordered == 'ranked':
        ...
        scores = all_feats(scores, ordered='ranked')
    ...
    if ordered == 'random':
        ...
        scores = all_feats(scores, ordered='random')
        ranked_features = scores['Features'].tolist()

        feedback = get_feedback(scores, ordered='random')
